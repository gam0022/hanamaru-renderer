...
pub fn execute_image(imgbuf: &mut ImageBuffer<Rgb<u8>, Vec<u8>>) {
    for _ in 0..config::BILATERAL_FILTER_ITERATION {
        bilateral_image(imgbuf, config::BILATERAL_FILTER_DIAMETER, config::BILATERAL_FILTER_SIGMA_I, config::BILATERAL_FILTER_SIGMA_S)
    }
}

fn bilateral_image(imgbuf: &mut ImageBuffer<Rgb<u8>, Vec<u8>>, diameter: u32, sigma_i: f64, sigma_s: f64) {
